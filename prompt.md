...
7. **Phone brief (optional — only when ntfy is configured):** If the kickoff says notifications are not configured, skip this step entirely. Otherwise write your own short phone summary, then send it via the notify helper (this calls the project’s `notify()` function). Do this yourself — do not leave summarization to the runner.
   ...
   3. Confirm the command succeeded before ending the run. If `npm run notify` exits 0 with no send (ntfy unset), treat that as success and continue.
