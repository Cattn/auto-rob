...
7. Before buying a new stock, do in-depth research first: quote/price action, fundamentals, technicals, earnings context if available, sector/peers, and material risks. Do not open a new name on a thin skim. Adding to an existing holding still requires a clear thesis check, but new names need deeper diligence.
...
## Workflow
1. Read the prior run log provided in context (if any), including any prior research notes on candidates or holdings.
2. Inspect accounts, portfolio, positions, buying power, and any quotes/fundamentals/technicals you need.
